...
**Key Decisions:**
- Data ownership split: client-owned (offline-first, push) vs coach-owned (server-authoritative, pull)
- set_by TEXT with CHECK constraint (not enum) for simplicity
- CoachGuard eagerly imported (not lazy) -- tiny component, must render before Coach chunk
- Network errors during role check fail closed (redirect) with toast warning
- invite_status is PostgreSQL ENUM for type safety and indexability
- UNIQUE(coach_id, email) unconditional -- one invite row per pair, upsert on resend
- Auto-link runs inside handle_new_user trigger (same transaction, atomic)
- EMAIL_FROM with fallback to Resend test domain
- security_invoker via CREATE OR REPLACE (not DROP+CREATE)
- Estimated count for pagination performance
- Quick stats hidden for multi-page rosters
- set_by defaults to 'self' when null from database
- JSON.parse(JSON.stringify()) for exercise snapshot and Json type compatibility
- Segmented control toggle for Clients/Templates/Check-ins views
- Coach workout on rest day uses fallback type push/dayNumber 1
- Self-directed fallback sets assignedWorkout to null for current session only
- Button groups (not Select) for 1-5 scale fields -- more mobile-friendly
- All check-in sections expanded by default
- localStorage for latest check-in (not Zustand) -- single read-only value
...
- Resend API key + Edge Function deployment required for invitations
- syncAllToCloud @deprecated (intentional backward compat)

**Technical Debt Incurred:**
- getTimeAgo helper duplicated in Coach.tsx and Home.tsx (intentional, small function)
- syncAllToCloud kept as @deprecated for backward compatibility
